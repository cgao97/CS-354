...
/* 
 * Function for allocating 'size' byte
...
void* Alloc_Mem(int size) {                      
    ...
    if (size > 0){
        ...
        while (ptr->size_status != 1){
            if (ptr->size_status == (size + 2)){
                ptr->size_status++;
                temp = ptr + ptr->size_status/4;
                temp->size_status += 2;
                return (ptr + 1);
            }
            if (((ptr->size_status & 1) == 0) && (ptr->size_status > size)){
                if (temp == NULL){
                    temp = ptr;
                }else if (ptr->size_status < temp->size_status){
                    temp = ptr;
                }
            }
            ptr += ptr->size_status/4;
            // printf("%d\n", ptr->size_status);
            // printf("%08x\n", (unsigned int)(ptr));
            }
        if (temp == NULL){
        ...
        ptr = temp + size/4;
        ptr->size_status = temp->size_status - size;
        ptr = temp + temp->size_status/4 - 1;
        ptr->size_status -= size;
        temp->size_status = size + 3;
        // Dump_Mem();
        return (temp + 1);
    }
    return NULL;
...
int Free_Mem(void *ptr) {
    ...
    if ((temp->size_status & 1) == 0){
        return -1;
    }
    blk_hdr *traverse = first_blk;
    while ((unsigned int)(traverse)< (unsigned int)(ptr)){
        if (traverse->size_status == 1){
            return -1;
        }
        traverse += traverse->size_status/4;
    }
    temp->size_status--;
    int count = temp->size_status - temp->size_status%8;
    if ((temp->size_status%8) == 0){
        temp--;
        count += temp->size_status;
        temp -= temp->size_status/4 - 1;
    }
    temp->size_status = count + 2;
    traverse = temp + temp->size_status/4;
    if ((traverse->size_status & 1) == 0){
        count += traverse->size_status - 2;
        temp->size_status = count + 2;
        traverse += traverse->size_status/4 - 1;
        traverse->size_status = count;
        return 0;
    }else{
        traverse->size_status -= 2;
        traverse--;
        traverse->size_status = count;
        return 0;
    }
